let hintTxt = document.querySelector(".hint-text b");
...
for (let i = 97; i <= 122; i++) {
    ...
    btn.setAttribute("id", String.fromCharCode(i).toUpperCase());
...
function resetGame() {
    ...
    correctLetters = [];
    wrongGuessCount = 0;
    hangmanImage.src = "media/hangman-0.svg";
    guessesText.innerText = `${wrongGuessCount} / ${maxGuesses}`;
    wordDisplay.innerHTML = currentWord.split("").map(() => `<li class="letter"></li>`).join("");
    
    // Disable all buttons except playAgainBtn
    keyboard.querySelectorAll("button:not(.play-again)").forEach(btn => {
        btn.disabled = false;
        btn.classList.remove("disabled");
    });

    // Enable the playAgainBtn
...
function buttonPressed() {
    clickedLetter = this.getAttribute("id");
